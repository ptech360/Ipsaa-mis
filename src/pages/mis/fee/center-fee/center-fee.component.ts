...
export class CenterFeeComponent implements OnInit {
  ...
  filterCharges(object) {
    ...
    if (object) {
      ...
      this.filterCharge = this.charges.filter(element => element.id == object.charge.id)
    } else {

      this.filterCharge = this.charges.filter(element => element.id != this.additionCharge);
    }
...
filterProgramList(){
  ...
  // get name() { return this.chargeListForm.get('name'); }
  ...
  saveProgramFee() {
      ...
      if ( this.editable) {
        ...
        this.adminService.editProgramFee(fd)
          .subscribe((res: any) => {
            ...
            this.selectedCenterDetails = this.selectedCenterDetails.filter(element => element.id != this.programFeeForm.value.id)
    ...
    if (this.selectedTab === 'Additional Charges') {
      ...
      if (this.editable) {
        ...
        this.adminService.editCharge({ "amount": this.additionalChargeForm.value.amount, "charge": this.additionalChargeForm.value.charge.id, "id": this.additionalChargeForm.value.id, "chargeId": this.additionalChargeForm.value.charge.id, "mode": 'Edit', "center": this.selectedCenter.id, "centerId": this.selectedCenter.id, })
          .subscribe((res: any) => {
            this.additionCharge = this.additionCharge.filter(element => element.id != res.id)
            ...
            this.alertService.errorAlert("sorry can't be edit")
        ...
        this.adminService.addNewCharge({ "amount": this.additionalChargeForm.value.amount, "charge": this.additionalChargeForm.value.charge.id, "chargeId": this.additionalChargeForm.value.charge.id, "mode": 'New', "center": this.selectedCenter.id, "centerId": this.selectedCenter.id, })
        .subscribe((res: any) => {
          this.saving = false;
    ...
    if (this.selectedTab === 'Charge List') {
      ...
      if (this.editable) {
        ...
        .subscribe((res: any) => {
          ...
          this.charges = this.charges.filter(element => element.id != this.selectedDetails.id)
  ...
  delete(id) {
    ...
    if (this.selectedTab === "Program Fee") {
      ...
      this.adminService.deleteProgramFee(id)
        .subscribe((res: any) => {
          this.selectedCenterDetails = this.selectedCenterDetails.filter(element => element.id != id)
    ...
    if (this.selectedTab === "Additional Charges") {
      this.adminService.deleteAdditionalCharge(id)
        .subscribe((res: any) => {
          this.additionCharge = this.additionCharge.filter(element => element.id != id)
          this.alertService.successAlert("delete successfuly")
        }, (err) => {
          this.alertService.errorAlert(err)
        })
